...
<template>
                ...
                <div class="col-lg-12">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <div class="btn-group col-lg-3" v-if="(route.fullPath === '/admin/documents')">
                            <button @click="getSelectedStatus()" type="button" class="btn" :class="[typeof selectedStatus === 'undefined' ? 'btn-secondary':'btn-default']">
                                <span class="mr-1">All</span>
                                <span class="badge badge-pill badge-info">{{ documentCount }}</span>
                            </button>

                            <button v-if="(route.name === 'admin.documents')" v-for="status in documentStatus" @click="getSelectedStatus(status.value)" type="button" class="btn" :class="[selectedStatus == status.value ? 'btn-secondary' : 'btn-default']">
                        ...
                        <div class="d-flex justify-content-md-center mb-2">
                            ...
                            <div v-if="selectedSearchByQuery === 'type'" class="col-md-3" @change="getDocuments()">
                                <select  v-model="searchQuery" class="form-control">
                                    <option v-for="document_type in document_types" :key="document_type.value" :value="document_type.value">{{ document_type.name }}</option>
                                </select>
                            </div>
                            ...
                            <table v-if="documentsCount > 0" class="table table-bordered">
                                ...
                                <tbody>
                                    <tr v-for="(document, index) in documents.data" :key="document.id">
                                        ...
                                        <td>
                                            <span class="badge" :class="`badge-${document.status.color}`">{{ document.status.name }}</span>
                                        </td>
                                        <td>
    ...
    <div class="modal fade" id="moveDocumentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select v-model="form.employee_id" id="employee_id" name="employee_id" class="form-control">
